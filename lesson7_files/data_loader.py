...
for analysis. It handles data validation, type conversions, and merging
...
class EcommerceDataLoader:
    ...
    def clean_orders_data(self) -> pd.DataFrame:
        ...
        Converts timestamp columns to datetime format and extracts date components.

        Returns:
            pd.DataFrame: Cleaned orders data with date components
        """
    ...
    def create_sales_dataset(
        ...
        Args:
            ...
            month_filter (int, optional): Filter by specific month (1-12)
        ...
        sales_data = self.processed_data['order_items'][
        ...
        # Apply time filters
        if year_filter:
            sales_data = sales_data[sales_data['purchase_year'] == year_filter].copy()

        if month_filter:
            sales_data = sales_data[sales_data['purchase_month'] == month_filter].copy()
        ...
        if ('order_delivered_customer_date' in sales_data.columns and
            'order_purchase_timestamp' in sales_data.columns):
...
def load_and_process_data(
    ...
    Returns:
        Tuple[EcommerceDataLoader, Dict[str, pd.DataFrame]]:
            Loader instance and processed data
    """
